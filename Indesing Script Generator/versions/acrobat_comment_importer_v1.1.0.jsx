...
(function () {
  ...
  var btnSelfCheck = footer.add("button", undefined, "Tripla checagem");
  ...
  function runSelfCheck() {
    state.selfCheckWarnings = [];

    verifyButtonHandlers();
    verifyParserSamples();
    verifyNormalization();

    var msg = "Tripla checagem concluída.\n\n";
    if (state.selfCheckWarnings.length) {
      msg += "Pontos de atenção:\n- " + state.selfCheckWarnings.join("\n- ");
    } else {
      msg += "Nenhum problema interno foi detectado nas checagens do script.";
    }
    alert(msg);
  }

  function verifyButtonHandlers() {
    var handlers = [
      { name: "Importar", ref: btnImport.onClick },
      { name: "Anterior", ref: btnPrev.onClick },
      { name: "Próximo", ref: btnNext.onClick },
      { name: "Localizar", ref: btnLocate.onClick },
      { name: "Atualizar busca", ref: btnRefresh.onClick },
      { name: "Alterar no script", ref: btnApplyDialog.onClick },
      { name: "Alterar na página", ref: btnApplyPage.onClick },
      { name: "Ignorar", ref: btnIgnore.onClick },
      { name: "Tripla checagem", ref: btnSelfCheck.onClick },
      { name: "Fechar", ref: btnClose.onClick }
    ];

    for (var i = 0; i < handlers.length; i++) {
      if (typeof handlers[i].ref !== "function") {
        state.selfCheckWarnings.push("Botão sem handler: " + handlers[i].name);
      }
    }
  }

  function verifyParserSamples() {
    var sample = ''
      + '<xfdf><annots>'
      + '<highlight page="2" subject="Revisao"><contents>Trocar "caza" por "casa"</contents><text>caza</text></highlight>'
      + '<text page="5"><contents>Ajustar legenda</contents></text>'
      + '</annots></xfdf>';
    var namespacedSample = ''
      + '<xfdf xmlns="http://ns.adobe.com/xfdf/">'
      + '<annots><highlight page="1"><contents>Trocar "aa" por "bb"</contents><text>aa</text></highlight></annots>'
      + '</xfdf>';

    try {
      var parsed = parseXfdfComments(sample);
      if (parsed.length !== 2) {
        state.selfCheckWarnings.push("O parser retornou uma quantidade inesperada de comentários no teste interno.");
      }
      if (!parsed[0].comment || parsed[0].pageNumber !== 3) {
        state.selfCheckWarnings.push("O parser não preservou comentário ou página no teste interno.");
      }
      var target = deriveTargetText(parsed[0]);
      var replacement = deriveReplacement(parsed[0]);
      if (target !== "caza" || replacement !== "casa") {
        state.selfCheckWarnings.push("A heurística de troca não reconheceu o padrão 'trocar X por Y'.");
      }
      var parsedNamespaced = parseXfdfComments(namespacedSample);
      if (parsedNamespaced.length !== 1) {
        state.selfCheckWarnings.push("O parser não reconheceu XFDF com namespace, comum nos arquivos do Acrobat.");
      }
    } catch (e) {
      state.selfCheckWarnings.push("Falha no teste interno do parser: " + e);
    }
  }

  function verifyNormalization() {
    var source = "Coração na página 10.";
    var normalized = normalizeForSearch(source);
    if (normalized.indexOf("coracao") < 0) {
      state.selfCheckWarnings.push("A normalização de acentos falhou no teste interno.");
    }
  }
})();
